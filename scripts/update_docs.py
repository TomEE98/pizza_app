...
old = '<section id="docs" class="page"><div class="eyebrow">Compliance library</div><h2>Docs</h2><div class="card"><h3>Your compliance pack</h3><p class="muted">Keep the documents you may need for an EHO inspection together. The prototype records document names and status; file uploads can be added next.</p><div id="docList"></div><button class="primary" onclick="addDocument()">+ Add document</button></div></section>'
new = '''<section id="docs" class="page">
...
function deletePremise(id){if(!confirm('Delete these premises? Existing photos will not be deleted.'))return;db.premises=db.premises.filter(x=>x.id!==id);persist();toast('Premises deleted')}
...
function addPremisePhoto(){if(!db.premises?.length){openModal('<h3>Add premises first</h3><p class="muted">Create at least one premises before adding a site photo.</p><button class="primary" onclick="closeModal();addPremise()">＋ Add premises</button>');return}openModal(`<h3>Add premises photo</h3><label>Premises</label><select id="mPhotoPremise">${db.premises.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}</select><label>Photo date</label><input id="mPhotoDate" type="date" value="${today()}"><label>Description</label><input id="mPhotoNote" placeholder="e.g. Front of trailer / hand-wash station"><label>Image</label><input id="mPhotoFile" type="file" accept="image/*" capture="environment"><p class="small muted">Photos are resized before being saved on this device.</p><button class="primary" onclick="savePremisePhoto()">Save photo</button>`)}
function savePremisePhoto(){const file=document.getElementById('mPhotoFile')?.files?.[0];if(!file){toast('Choose an image first');return}const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=1200,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.width*scale));canvas.height=Math.max(1,Math.round(img.height*scale));canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);const data=canvas.toDataURL('image/jpeg',.78);db.premisePhotos=Array.isArray(db.premisePhotos)?db.premisePhotos:[];db.premisePhotos.unshift({id:Date.now()+Math.random(),premiseId:document.getElementById('mPhotoPremise').value,date:document.getElementById('mPhotoDate').value||today(),note:(document.getElementById('mPhotoNote').value||'').trim(),image:data,createdAt:new Date().toISOString()});try{closeModal();persist();setDocsTab('premises');toast('Premises photo saved')}catch(e){toast('Photo is too large to save on this device')}};img.src=reader.result};reader.readAsDataURL(file)}
function deletePremisePhoto(id){if(!confirm('Delete this premises photo?'))return;db.premisePhotos=db.premisePhotos.filter(x=>x.id!==id);persist();toast('Photo deleted')}
function renderPremisePhotos(){const el=document.getElementById('premisePhotosList');if(!el)return;const photos=Array.isArray(db.premisePhotos)?db.premisePhotos:[];if(!photos.length){el.innerHTML='<div class="docs-empty">No premises photos saved yet.<br>Use “Add photo” to upload dated site evidence.</div>';return}el.innerHTML='<div class="photo-grid">'+photos.map(x=>{const p=(db.premises||[]).find(y=>String(y.id)===String(x.premiseId));return `<div class="premise-photo"><img src="${x.image}" alt="${x.note||'Premises photo'}"><div class="premise-photo-body"><div class="premise-photo-date">${fmtDate(x.date)}</div><div class="premise-photo-note">${p?p.name:'Premises removed'}${x.note?' · '+x.note:''}</div><button class="secondary danger" style="width:100%;margin-top:9px" onclick="deletePremisePhoto(${x.id})">Delete photo</button></div></div>`}).join('')+'</div>'}
'''
if marker not in s:
    raise SystemExit('renderDocs marker not found')
...
print('Docs premises update applied')
