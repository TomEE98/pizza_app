...
#recordFields .prep-clean-actions{margin-top:14px}
#recordFields .prep-clean-actions .primary{margin-top:0}
...
(function(){
  ...
  function updateSummary(){
    const el=document.getElementById('prepChecklistSummary');
    if(!el)return;
    const n=checkedCount();
    el.innerHTML='<strong>'+n+' / '+checks.length+'</strong> checks completed'+(n===checks.length?' · Ready to sign':' · Complete all checks before signing');
  }
  function renderPrep(){
    const fields=document.getElementById('recordFields');
    if(!fields)return;
    const rows=checks.map(c=>`<label class="prep-clean-item"><input id="${c[0]}" type="checkbox"><span class="prep-clean-box">✓</span><span class="prep-clean-copy"><b>${c[1]}</b><small>${c[2]}</small></span></label>`).join('');
    fields.innerHTML=`<div class="card prep-clean-card"><div class="prep-clean-head"><h3>Prep & cleaning checks</h3><p>This single checklist records your preparation, hygiene and cleaning checks for the service.</p></div><div class="prep-clean-list">${rows}</div></div><div id="prepChecklistSummary" class="prep-clean-summary">0 / ${checks.length} checks completed · Complete all checks before signing</div><div class="prep-clean-failure"><b>Something not right?</b><span class="small muted">Leave the affected check unticked and explain what you did below. The saved record will show exactly which checks were completed and any corrective action.</span></div><label style="margin-top:14px">Corrective action / notes</label><textarea id="prepAction" placeholder="Record any issue, action taken, food isolated/disposed of, or other notes..."></textarea><div class="prep-clean-sign"><label>Completed by</label><input id="prepBy" placeholder="Name"></div>`;
    fields.querySelectorAll('.prep-clean-item input').forEach(x=>x.addEventListener('change',updateSummary));
    updateSummary();
  }

  const originalRender=window.renderRecordFields;
  window.renderRecordFields=function(type){
    if(type===PREP_TYPE){
      document.getElementById('addTitle').textContent='Prep & cleaning checks';
      renderPrep();
      document.getElementById('add')?.classList.remove('temperature-entry');
      return;
    }
    return originalRender(type);
  };

  const originalSave=window.saveRecord;
  window.saveRecord=function(){
    const type=document.querySelector('.add-tab.active')?.dataset.type||'';
    if(type!==PREP_TYPE)return originalSave();
    const date=document.getElementById('entryDate').value||today();
    const time=document.getElementById('entryTime').value||new Date().toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    const signer=(document.getElementById('prepBy')?.value||'').trim();
    if(!signer){toast('Enter the name of the person completing the checks');return}
    const status={};
    checks.forEach(c=>status[c[0]]=!!document.getElementById(c[0])?.checked);
    const completed=checks.filter(c=>status[c[0]]).length;
    if(completed<checks.length){
      const action=(document.getElementById('prepAction')?.value||'').trim();
      if(!action){toast('Add a corrective action or note for the unticked checks');return}
    }
    const r={id:Date.now(),type:PREP_TYPE,date,time,data:{
      checklistVersion:'Prep & cleaning checklist v1',
      checklist:status,
      checklistItems:checks.map(c=>({id:c[0],label:c[1],checked:!!status[c[0]]})),
      completedChecks:completed,
      totalChecks:checks.length,
      outcome:completed===checks.length?'All checks completed':'Attention required',
      action:(document.getElementById('prepAction')?.value||'').trim(),
      notes:(document.getElementById('prepAction')?.value||'').trim(),
      signer:signer,
      completedBy:signer
    }};
    db.records.unshift(r);
    persist();
    toast('Prep & cleaning checks saved');
    go('records');
  };

  // Make the Today core status treat Prep & cleaning as one combined log; there is no separate cleaning record requirement.
  const originalRenderToday=window.renderToday;
  window.renderToday=function(){
    const out=originalRenderToday();
    const rs=db.records.filter(r=>r.date===today());
    const prepDone=rs.some(r=>r.type==='Prep record');
    const clean=document.getElementById('coreClean');
    const prep=document.getElementById('corePrep');
    if(clean){clean.style.display='none';}
    if(prep){prep.querySelector('span:last-child')?.replaceChildren(document.createTextNode('Prep & clean'));prep.classList.toggle('done',prepDone)}
    const orderDone=['Opening checklist','Temperature check','Prep record','Closing checklist'].filter(t=>rs.some(r=>r.type===t)).length;
    const count=document.getElementById('todayProgressCount');if(count)count.textContent=orderDone+' / 4';
    const text=document.getElementById('todayProgressText');if(text)text.textContent=(4-orderDone)+' core checks remaining';
    const bar=document.getElementById('todayProgressBar');if(bar)bar.style.width=((orderDone/4)*100)+'%';
    const pct=document.getElementById('todayProgressPercent');if(pct)pct.textContent=Math.round(orderDone/4*100)+'%';
    return out;
  };
  const originalComplete=window.completeRemainingChecks;
  window.completeRemainingChecks=function(){
    const order=[['Opening checklist',document.getElementById('coreOpening')?.classList.contains('done')],['Temperature check',document.getElementById('coreFridge')?.classList.contains('done')],['Prep record',document.getElementById('corePrep')?.classList.contains('done')],['Closing checklist',document.getElementById('coreClosing')?.classList.contains('done')]];
    const next=order.find(x=>!x[1]);if(next){quickToday(next[0]);}else toast('All core checks are complete');
  };
  const originalSpeak=window.speakToday;
  window.speakToday=function(){
    if(typeof originalSpeak!=='function')return;
    const rs=db.records.filter(r=>r.date===today());
    const done=['Opening checklist','Temperature check','Prep record','Closing checklist'].filter(t=>rs.some(r=>r.type===t)).length;
    if(!('speechSynthesis' in window)){toast('Voice is not available on this device');return}
    const text=`Blasko's Pizza. Today is ${new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}. You have completed ${done} of 4 core checks. ${4-done} remaining.`;
    speechSynthesis.cancel();speechSynthesis.speak(new SpeechSynthesisUtterance(text));
  };

  function boot(){
    // Keep the existing Prep tab but make its label explicit.
    document.querySelector('.add-tab[data-type="Prep record"]')?.replaceChildren(document.createTextNode('Prep & clean'));
    if(document.querySelector('.add-tab.active')?.dataset.type===PREP_TYPE)renderPrep();
  }
...
# Insert before the last real </body>. The app contains escaped </body> text inside a print template, so use rfind only.
pos=s.rfind('</body>')
